...
class L1RAConfig(LoraConfig):
    ...
    eta_c: float = field(default=1e-2, metadata={"help": "The decoupled learning rate for the gate vectors."})
    rank_update_ratio: int = field(default=0.1, metadata={"help": "Ratio of training steps between each rank update."})
    prune_threshold: float = field(default=1e-6, metadata={"help": "Threshold under which ranks are pruned."})
